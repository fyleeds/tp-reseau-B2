...
async def connect_server():
    ...
    # Assurez-vous que le socket est fermé même en cas d'erreur
        ...
        sys.exit(1)
async def client_chat(connect_server, prompt):
...
def input_pseudo(s):
    pseudo_message = "Hello|" + input("Send nickname please : ")
    try :
        ## une ligne pour send le message (l'envoyer)
        s.send(pseudo_message.encode())
    except Exception as e:
...
def main():
    ...
    #first connexion with sync client
    first_connexion()

    #start async client
    prompt = "Please enter your message : \n"
    asyncio.run(client_chat(connect_server(), prompt))
